...
export class AssetViewerScene extends Phaser.Scene {
  private assets!: PilotAssetRuntime;
  ...
  create(): void {
    this.assets = createPilotAssetRuntime(this);
  ...
  private rebuildPreview(): void {
    ...
    if (this.state.assetId === AUTHORED_ASSET_IDS.homeIsland) {
      this.homeVisual = createAuthoredHomeIslandVisual(this, this.assets);
      ...
      this.shipRenderer = new ShipRenderer(this, this.assets);
    } else {
      this.shoalVisual = createAuthoredFishingShoalVisual(this, this.assets);
  ...
  private drawGuides(): void {
    ...
    const metadata = this.assets.metadata(this.state.assetId);
    if (!metadata) return;
  ...
  private mountControls(): void {
    ...
    if (diagnostics) diagnostics.value = this.assets.diagnostics.length === 0
      ? `${ASSET_IDS.length} packages loaded with complete textures.`
      : this.assets.diagnostics.map(({ assetId, message }) => `${assetId}: ${message}`).join("\n");
  }
  ...
  private installDebugApi(): void {
    window.__WAYFINDERS_ASSET_VIEWER__ = {
      ...
      diagnostics: () => this.assets.diagnostics.map(({ assetId, message }) => `${assetId}: ${message}`),
    };
  }
