...
connectivity, and service-anchor search use the grid's explicit topology and
deduplicated canonical neighbours. Lifted bounds are split through that same
topology and return unique row-major canonical indices. Query diagnostics
report the indexed candidate set examined so tests can prevent a new feature
from quietly introducing another full-world scan.
