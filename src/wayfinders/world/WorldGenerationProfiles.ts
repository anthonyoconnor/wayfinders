...
function createProfile(
  ...
  return Object.freeze({
    ...
    minimumChannel: Object.freeze({
      ...
      edgeMargin: config.islands.edgeMargin,
...
function generationSettingsJson(config: PrototypeConfig): string {
...
export function worldGenerationProfileIdForConfig(config: PrototypeConfig): string {
  const settings = generationSettingsJson(config);
  for (const profile of Object.values(WORLD_GENERATION_PROFILES)) {
    if (generationSettingsJson(profile.config) === settings) return profile.id;
  }
  return "custom";
}

/** Small deterministic FNV-1a fingerprint; this is an identity aid, not cryptography. */
export function worldGenerationSettingsFingerprint(config: PrototypeConfig): string {
  const bytes = new TextEncoder().encode(generationSettingsJson(config));
  let hash = 0x811c9dc5;
